...
function getGraphQLQuery(bodyStr) {
  ...
  try {
    ...
    if (query !== undefined) {
      const queryRegex = /(query|mutation) ([^\(]*)\(/g;
      const queryMatches = queryRegex.exec(query)
      if (queryMatches != null) {
        const queryOrMutation = queryMatches[1]
        const queryName = queryMatches[2]
        return {
...
function normalizeInfo(id, info) {
  ...
  const query = getGraphQLQuery(info.reqBody)
  if (query != null) {
    singleRecord.graphQL_name = query.name;
    singleRecord.graphQL_type = query.type;
  }
